...
```pug
+picture('srcset', 'src', 'alt')
```

На первом месте указываете адрес обычной картинки, на втором - сжатый webp - файл. На третьем, соответственно, атрибут alt.
