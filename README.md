...
Контейнерам layout, content и row - прописаны grid-правила, позволяющие начинать работу без базовой настройки сетки.
...
Подключается в index.pug, в block content
...
Сайдбар тесно связан с layout-none. Он не будет корректно работать в container или layout. К тому же, layout-none обязателен для того, чтобы у сайдбара не было белых полей при контрастном фоне.

Подключается в index.pug, в block content.

Активируется, если вы поместите элемент с классом content-sidebar внутрь layout-none.
