...
1. адаптивность из коробки. Конейнерам layout, content и row, прописаны grid-правила, позволяющие начинать работу без базовой настройки сетки, если у проекта нет нестандартных элементов, вылезающих за границы контейнеров.
...
Аналог популярного контейнера .container, но задаёт глобальный контейнер, определяющий размеры шапки, контентной части и подвала. Задаётся через grid-template-rows и grid-template-columns. grid-template-columns содержит переменную $container.

Ширину контейнера layout определяет переменная ``$container`` в ``./src/styles/helpers/_variables.scss``

> Если в вашем проекте много элементов с фоном, выходящим за границы контейнера:

Уберите ```grid-template-columns: minmax(250px, $container);``` и ```justify-content: center``` из верстки layout и класс layout из:
...
И вместо него пропишите ширину контейнера... в .container.

#### адаптация layout

### container

Нужен в частных случаях, когда надо вместо общей ширины layout вставить ограничивающий контейнер. Подходит для случаев, когда нужен однородный фон без белых полей по бокам.

> ```.container``` рекомендуется вставлять после объявления семантического или смыслового блока, например:
...
Кстати, про количество колонок. Если колонки больше 320, они автоматически подстроятся под ширину экрана.
...
Все файлы и импорты этих библиотек уже вставлены, при необходимости их надо лишь раскомментировать.
...
Подключение компонента отличается от других тем, что он подключается в index.pug под блоком footer:
...
Все остальные файлы подключаются также, как и другие компоненты.
