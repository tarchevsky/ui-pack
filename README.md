...
Для быстрого использовния picture существует ёмкий миксин:

```pug
+picture('jpg', 'webp', 'alt')
```

На первом месте указываете адрес обычной картинки, на втором - сжатый webp - файл. На третьем, соотвественно, атрибут alt.
